...
\title{Identifying genetic loci under selective pressure using a
  ...
  classifier} \author{Toby Dylan Hocking}
...
allele frequencies 
$$\pi_i=\alpha_{i1}(1)=...=\alpha_{iP}(1)$$
we draw from a truncated $\beta(0.7,0.7)$ distribution
...
data. Generally, if we use $x$ to signify the observed data and
$\theta$ to denote the model parameters, we can write the posterior
distribution as
\begin{equation}
\label{bayes}
...
(\autoref{loci-over-time}). This plot shows 2 loci under balancing
selection, 2 loci not under selection, and 2 loci under positive
selection.
...
evolve toward an allele frequency of 50\%, accurately simulating the
...
\fig{notbeta}{Scatter plot matrix for various values of ancestral
  allele frequency $\pi_i$ for a simulated data set (actual simulated
  values indicated by the row/column labeled simulated). Models using
...
individuals, so data from SNP microarrays is necessarily biased to
favor loci which are polymorphic in these individuals. This phenomenon
is called the ``ascertainment bias'' in the literature.
...
To identify the threshhold values of empirical risk minimization, we
...
The research groups of G\'en\'etique Animale et Biologie Int\'egrative
(GABI) and Population, Statistique et G\'enome (PSGen) at l'Institut
National de la Recherche Agronomique (INRA) in Jouy-en-Josas
